/*
 * (full) bin 3d data into 2D (ascii or binary files). View by xmgrace.
 *
 */
...
int main(int argc, char **argv) {
  ...
  int col, row, ncol, nrow, val;
  double *data, *data2, tmp;
  FILE *fp, *fp2;
  char asd[128];

  if (argc < 7 || argc > 8) {
    fprintf(stderr, "Usage: slice2 [-a|-b] [-x|-y] val ncol nrow file [bkg_file]\n");
    exit(1);
  }
